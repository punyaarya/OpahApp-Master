...
//                Log.d("xxx", "driver infor: " + tmp.toString());
    ...
    public static void registerCustomerInfoToFirebase() {
        //Menulis sementara kepada pelanggan di sini, setelah pendaftaran selesai akan menulis
        FirebaseDatabase database = FirebaseDatabase.getInstance();
    ...
    public static void getCustomerLocation(String customerRequestId, String key) {
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().
                getReference("driversWorking").child(getUid()).child(customerRequestId);//.child(key);

        GeoFire geoFire = new GeoFire(databaseReference);
        geoFire.getLocation(key, new LocationCallback() {
            @Override
            public void onLocationResult(String key, GeoLocation location) {
                if(key.equals("startLoc"))
                    Driver.getInstance().setStartLocation(location.latitude, location.longitude);
                else
                    Driver.getInstance().setEndLocation(location.latitude, location.longitude);
            }
